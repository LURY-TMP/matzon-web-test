    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#0a0a0c', color: '#fff' }}>
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '72px 16px 12px' }}>
        <nav style={{ display: 'flex', gap: 20, overflowX: 'auto' }}>
          {tabs.map(tab => (
            <button key={tab.label} onClick={() => { setActiveTab(tab.label); router.push(tab.href); }} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 13, fontWeight: 600, color: activeTab === tab.label ? '#fff' : '#808085', borderBottom: activeTab === tab.label ? '2px solid #2563FF' : '2px solid transparent', paddingBottom: 4, whiteSpace: 'nowrap' }}>{tab.label}</button>
          ))}
        </nav>
      </header>
      <section style={{ position: 'relative', paddingBottom: 48 }}>
        ...
        <div style={{ padding: '0 16px', marginTop: -24 }}>
          ...
          <div style={{ display: 'flex', gap: 12, marginBottom: 24 }}>
            <button onClick={handleCTA} style={{ flex: 1, background: 'linear-gradient(135deg, #2563FF, #7C3AED)', color: '#fff', border: 'none', padding: '14px', borderRadius: 30, fontSize: 15, fontWeight: 700, cursor: 'pointer' }}>{isLoggedIn ? 'Ir para Dashboard' : 'Comecar agora'}</button>
            <button onClick={handleCTA} style={{ background: 'rgba(255,255,255,0.08)', color: '#fff', border: 'none', width: 48, height: 48, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0 }}><Plus style={{ width: 20, height: 20 }} /></button>
          </div>
